...
const directoryPath = "./src/pages/admins/products"; // Update this with your source directory
...
function replacePatternInFile(filePath) {
  ...
  const pattern = /label="(.*?)"/g; // Matches <base-label>Any Text</base-label>
  content = content.replace(pattern, (match, value) => {
    if (value.startsWith("$t(")) {
      return match;
    }
    const camelCaseKey = toCamelCase(value);
    const replacement = `:label="$t('label.${camelCaseKey}')"`;
