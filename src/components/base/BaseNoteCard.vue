...
export default {
  ...
  computed: {
    ...
    getMessage() {
      ...
      let attributes = [];
      if (this.type === "updated" && this.options) {
        Object.keys(this.options).forEach((key) => {
          let item = this.options[key];
          let label = this.$core.capitalize(
            key.replace("_id", "").replaceAll("_", " ")
          );
          if (key === "password") {
            attributes.push(`${label} changed`);
          } else {
            attributes.push(
              `${label} changed from ${item.previous} to ${item.current}`
            );
          }
        });
