<template>
  ...
  >
    <template v-slot:body>
      ...
      <base-input v-model="url" type="url" placeholder="https://" />
    </template>
...
export default {
  ...
  methods: {
    ...
    onDialogHide() {
      console.func(
        "components/file-from-url:methods.onDialogHide()",
        arguments
      );
      this.$emit("hide");
    },
  ...
  computed: {
    ...
    title() {
      if (this.type === "photo") {
        return this.$t("addPhotoFromUrl");
      } else {
        return this.$t("embedVideo");
      }
    },
    label() {
      if (this.type === "photo") {
        return this.$t("pasteImageUrl");
      } else {
        return this.$t("youTubeOrVimeoUrl");
      }
    },
  },
};
