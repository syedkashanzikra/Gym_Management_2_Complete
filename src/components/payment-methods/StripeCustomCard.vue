    ...
    <q-card-section :class="{ 'q-pa-none': dense }">
      ...
      <div class="row q-col-gutter-lg">
        <div class="col-xs-12 col-sm-4">
          <base-label>{{ $t("cardNumber") }}</base-label>
          <stripe-field element="number" :error="error" />
        </div>
        <div class="col-xs-12 col-sm-4">
          <base-label>{{ $t("expiryDate") }}</base-label>
          <stripe-field element="expiry" :error="error" />
        </div>
        <div class="col-xs-12 col-sm-4">
          <base-label>{{ $t("cvcCardVerificationCode") }}</base-label>
...
export default {
  ...
  async mounted() {
    ...
    const cardNumberElement = (this.cardNumberElement = elements.create(
      "cardNumber",
      {
        style: baseStyle,
      }
    ));
    const cardExpiryElement = (this.cardExpiryElement = elements.create(
      "cardExpiry",
      {
        style: baseStyle,
      }
  ...
  computed: {
    ...mapState(useAppStore, ["stripePayment"]),
    stripe_key() {
      return this.stripePayment?.API_KEY ?? process.env.STRIPE_KEY;
    },
    label() {
      return (
        this.btnLabel ||
        this.$t("payAmount", { amount: this.$core.money(this.amount) })
      );
    },
    paymentMethods() {
      const methods = [
